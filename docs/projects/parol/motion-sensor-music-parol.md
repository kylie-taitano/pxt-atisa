...
In the ``||logic:else||`` part (when lights are off), let's turn them on! Drag ``||parol:set all to color||`` set to gold, then ``||parol:show lights||``. This will light up your parol!
